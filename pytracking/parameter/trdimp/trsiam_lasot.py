...
def parameters():
    ...
    params.update_classifier = True   # In order to update the Transformer memory
    ''' In the Siamese pipeline, we do not optimize the template '''
    params.net_opt_iter = 0
    params.net_opt_update_iter = 0
    params.net_opt_hn_iter = 0

    # transformer memory update
    # do not update the transformer memory in long-term benchmarks
    params.transformer_skipping = 1e-1 
    params.transformer_memory_size = -1
    ...
    params.net = NetWithBackbone(net_path='/home/marq/Desktop/CSA/pytracking/networks/trdimp_net.pth.tar', use_gpu=params.use_gpu)
