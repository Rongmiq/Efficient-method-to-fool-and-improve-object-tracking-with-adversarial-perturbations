...
from attack_utils import adv_attack_dimp
...
# from GAN_utils_template_search import *

# Running a tracker using PyTracking Toolkit
# class TrDiMP(BaseTracker):
#
...
#
#         #====================================================== GAN  adv  =========================================================#
#         # Extract backbone features with adv.
#         init_backbone_feat = self.generate_init_samples_adv(im,GAN_1)
#         # ====================================================== GAN  adv  ========================================================#
#
#
...
#         #====================================================== GAN  adv  =========================================================#
#         # Extract backbone features with adv.
#         # backbone_feat, sample_coords, im_patches = self.extract_backbone_features_adv(im, self.get_centered_sample_pos(),
#         #                                                                           self.target_scale * self.params.scale_factors,
#         #                                                                           self.img_sample_sz,GAN_1)
#         # ====================================================== GAN  adv  ========================================================#
#
...
#
#     def extract_backbone_features_adv(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor,GAN):
#         im_patches, patch_coords = sample_patch_multiscale(im, pos, scales, sz,
#                                                            mode=self.params.get('border_mode', 'replicate'),
#                                                            max_scale_change=self.params.get('patch_max_scale_change',
#                                                                                             None))
#         # print('im_patches',im_patches.shape) #[1,3,224,224]
#         im_adv_0 = torch.zeros_like(im_patches)
#         for i in range(im_patches.shape[0]):
#             im_adv_0[i] = adv_attack_dimp(im_patches[i].unsqueeze(0).cuda(), GAN,
#                                           img_size=(im_patches.shape[-1], im_patches.shape[-1]))
#         with torch.no_grad():
#             backbone_feat = self.net.extract_backbone(im_adv_0)
#         return backbone_feat, patch_coords, im_adv_0
#
...
#
#     def generate_init_samples_adv(self, im: torch.Tensor,GAN) -> TensorList:
#         """Perform data augmentation to generate initial training samples."""
#
#         mode = self.params.get('border_mode', 'replicate')
#         if mode == 'inside':
...
#                                               self.transforms)
#
#         # print('im_pa',im_patches.size()) [10,3,224,224]
#         # clean imgs and GAN(imgs) to get adversarial examples.
#         im_adv_0 = torch.zeros_like(im_patches)
#         for i in range(im_patches.shape[0]):
#             im_adv_0[i] = adv_attack_dimp(im_patches[i].unsqueeze(0).cuda(),GAN,img_size=(im_patches.shape[-1],im_patches.shape[-1]))
#
#         # examples
#         # Extract initial backbone features
#         with torch.no_grad():
#             init_backbone_feat = self.net.extract_backbone(im_adv_0)
#         return init_backbone_feat
#
...
class TrDiMP(BaseTracker):
    ...
    def track(self, image):
        ...
        # Extract classification features
        x_clf = self.get_classification_features(backbone_feat)
        # print('x',x_clf.shape) #[1,512,14,14]
        decoded_x, test_x = self.transformer_decoder(x_clf)
        # print('x', decoded_x.shape) [196, 1, 512]]
        # print('x1', test_x.shape) [1,512,14,14]
        ...
        if update_flag and self.params.get('update_classifier', False):
            ...
            if (self.frame_num - 1) % self.params.transformer_skipping == 0:
                ...
                if self.x_clf.shape[0] < self.params.transformer_memory_size:
                    self.transformer_label = torch.cat([cur_tf_label.unsqueeze(1).cuda(), self.transformer_label],
                                                       dim=0)
                    self.x_clf = torch.cat([x_clf, self.x_clf], dim=0)
                else:
                    self.transformer_label = torch.cat(
                        [cur_tf_label.unsqueeze(1).cuda(), self.transformer_label[:-1, ...]], dim=0)
                    self.x_clf = torch.cat([x_clf, self.x_clf[:-1, ...]], dim=0)
    ...
    def transformer_decoder(self, sample_x: TensorList):
        """Transformer."""
        with torch.no_grad():
            decoded_feat, out_feat = self.net.classifier.transformer.decoder(sample_x.unsqueeze(0),
                                                                             memory=self.transformer_memory,
                                                                             pos=self.transformer_label,
                                                                             query_pos=None)  ######### self.transformer_label
        return decoded_feat, out_feat
    ...
    def init_classifier(self, init_backbone_feat):
        ...
        # mask in Transformer
        self.transformer_label = prutils.gaussian_label_function(target_boxes.cpu().view(-1, 4), 0.1,
                                                                 self.net.classifier.filter_size,
                                                                 self.feature_sz, self.img_sample_sz,
                                                                 end_pad_if_even=False)
        self.transformer_label = self.transformer_label.unsqueeze(1).cuda()
        ...
        for i in range(x.shape[0]):
            _, cur_encoded_feat = self.net.classifier.transformer.decoder(x[i, ...].unsqueeze(0).unsqueeze(0),
                                                                          memory=self.transformer_memory,
                                                                          pos=self.transformer_label, query_pos=None)
            if i == 0:
                encoded_feat = cur_encoded_feat
            else:
                encoded_feat = torch.cat((encoded_feat, cur_encoded_feat), 0)
