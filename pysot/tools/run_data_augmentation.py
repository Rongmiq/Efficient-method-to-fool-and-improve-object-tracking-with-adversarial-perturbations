...
'''GAN'''
from GAN_utils_data_augmentation import *
model_name = 'G_template_search_L2500'+ '_ts'
...
parser.add_argument('--adv', default= True, action='store_true',
        help='whether adv')
parser.add_argument('--save_path', type=str, default='../clean_adv_imgs',
        help='config file')
...
def main():
    ...
    if args.dataset in ['VOT2016', 'VOT2018', 'VOT2019']:
        ...
        for v_idx, video in enumerate(dataset):
            ...
            for idx, (img, gt_bbox) in enumerate(video):
                ...
                if idx == frame_counter:
                    ...
                    '''GAN'''
                    tracker.attack_template_for_data_augmentation(img, gt_bbox_,GAN_0)
                    pred_bbox = gt_bbox_
                    ...
                    if args.adv:
                        if 'car' in args.tracker_name:
                            outputs = tracker.attack_search_for_data_augmentation(img, GAN_0, hp)
                        else:
                            outputs = tracker.attack_search_for_data_augmentation(img, GAN_0)
                    else:
                        if 'car' in args.tracker_name:
            ...
            # save results
            video_path = os.path.join('results', args.dataset, model_name+args.tracker,
                    'baseline', video.name)
        ...
        # OPE tracking
        for v_idx, video in enumerate(dataset):
            model_path = os.path.join('results', args.dataset, model_name + args.tracker)
