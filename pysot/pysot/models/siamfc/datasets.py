...
class Pair(Dataset):
    ...
    def __getitem__(self, index):
        ...
        w_z_tar = box_z[2]
        h_z_tar = box_z[-1]
        z_x_rate = float((w_z_tar * h_z_tar) / (w_z_pic * h_z_pic))

        w_x_pic = x.shape[0]
        h_x_pic = x.shape[1]
        w_x_tar = box_x[2]
        h_x_tar = box_x[-1]
        x_z_x_rate = float((w_x_tar * h_x_tar) / (w_x_pic * h_x_pic))

        # ------------------------------------------------------------
        if self.transforms is not None:
            item = self.transforms(z=z, x=x, box_z=box_z, box_x=box_x)  # z,x
            item += (z_x_rate, x_z_x_rate)
