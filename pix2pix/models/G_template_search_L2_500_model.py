...
parser.add_argument('--tracker_name', type=str, default='siamcar_r50',help='tracker name such as siamcar, siamgat ....')
args = parser.parse_args()

class GtemplatesearchL2500Model(BaseModel):
    ...
    def transform_512_127(self,patch_clean1,target_sz=(127,127)):
        '''resize to (512,512)'''
        patch512_clean1 = torch.nn.functional.interpolate(patch_clean1, size=(512, 512), mode='bilinear')
        patch512_adv1 = patch512_clean1 + self.netG(patch512_clean1)  # Residual form: G(A)+A
        patch_adv1 = torch.nn.functional.interpolate(patch512_adv1, size=target_sz, mode='bilinear')
        patch_adv127 = patch_adv1 * 127.5 + 127.5
        return patch_adv127
    ...
    def forward(self,target_sz=(255,255)):
        ...
        template512_clean1 = torch.nn.functional.interpolate(self.template_clean1, size=(512, 512), mode='bilinear')
        template512_adv1 = template512_clean1 - self.netG(template512_clean1)  # Residual form: G(A)+A
    ...
    def backward_G(self):
        ...
        # Second, G(A) = B
        self.loss_G_L2 = (self.criterionL2(self.search_adv1, self.search_clean1) + self.criterionL2(self.template_adv1, self.template_clean1)) * self.opt.lambda_L2
        self.loss_G_Linf = self.linf(self.search_adv255, self.search_clean255) + self.linf(self.template_adv255, self.template_clean255)
        n, c, w, h = self.clean_responses.shape
        # weight = 1-self.tensor0_1(self.clean_responses)
