...
def adv_attack_template_or_search(img_tensor,GAN):
...
def adv_attack_template_S(img_tensor, GAN, target_sz=(127,127)):
    ...
    '''step2: pass to G'''
    with torch.no_grad():
        img_adv = GAN.transform_template(img_tensor,target_sz)
        return img_adv


def adv_attack_dimp(img_tensor,GAN,img_size=(255,255)):
    '''adversarial attack to template patches and search region'''
    '''input: pytorch tensor(0,255) ---> output: pytorch tensor(0,255)'''
    '''step1: Normalization'''
    img_tensor = normalize(img_tensor)
    '''step2: pass to G'''
    with torch.no_grad():
        GAN.search_clean1 = img_tensor
        GAN.num_search = img_tensor.size(0)
        GAN.forward(img_size)
    img_adv = GAN.search_adv255
    return img_adv

def adv_attack_search_gat(img_tensor,GAN,search_sz=(287,287)):
    '''adversarial attack to search region'''
    '''input: pytorch tensor(0,255) ---> output: pytorch tensor(0,255)'''
    '''step1: Normalization'''
    img_tensor = normalize(img_tensor)
